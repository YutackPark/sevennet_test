...
def greeting():
    ...
    global RTOL, ATOL
    print(f'SevenNet version {sevenn.__version__}')
...
def sevenn_infer_to_atoms(inference_folder):
    ...
    for _, row in enumerate(per_graph):
        ...
        for _, frow in enumerate(per_atom_data):
            ...
            atomic_energies.append(float(frow[KEY.ATOMIC_ENERGY]))
        species = np.array(species)
        pos = np.array(pos)
        forces = np.array(forces)
        calc_res['forces'] = forces
        calc_res['energies'] = atomic_energies
        atoms = ase.Atoms(
...
def compare_atoms(atoms1, atoms2, rtol=1e-5, atol=1e-8):
    ...
    try:
        ae1 = atoms1.get_potential_energies()
        ae2 = atoms2.get_potential_energies()
        if not np.allclose(ae1, ae2, rtol=rtol, atol=atol):
            raise ValueError('atoms1 and atoms2 have different atomic_energies')
    except calculators.calculator.PropertyNotImplementedError:
        print('One of atoms does not have atomic energies, skip atomic energy comparison')
...
def sevenn_calculator_test(checkpoint, atoms, device):
    ...
    cal = SevenNetCalculator(checkpoint, device=device)
    ref_atoms = copy.deepcopy(atoms)
    atoms.calc = cal  # calculator resutls reset, it will be re-evaluated
